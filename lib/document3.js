...
Document2.prototype.syncTags = function(jsonTags, defaultType) {
	var context = {
	...
	if (util.isArray(jsonTags)) {
		...
		jsonTags.forEach(function(jsonTag) {
			if (!tag.$$tagId) {
				...
				self._createTag(context, jsonTag, defaultType);
				return;
			}
...
Document2.prototype._createTag = function(context, json, ebmlID) {
...
Document2.prototype._updateTag = function(context, tag, json) {
	...
	for ( var name in json) {
		...
		if (!tValue) {
			if (util.isArray(jValue)) {
				for (var i = 0; i < jValue.length; i++) {
					var newTag2 = this._createTag(context, jValue[i], ebmlID);
			...
			var newTag = this._createTag(context, jValue, ebmlID);
		...
		if (util.isArray(jValue)) {
			...
			for (; j < jValue.length; j++) {
				var newTag3 = this._createTag(context, jValue[j], ebmlID);
