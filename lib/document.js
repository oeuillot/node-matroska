...
Document.prototype.createTag = function(ebmlID, start, length) {
	return this.createElement(ebmlID, start, length);
};

Document.prototype.createElement = function(ebmlID, start, length) {
	var tag;

	if (ebmlID === schema.byName.Segment) {
		if (!Segment3) {
			Segment3 = require('./segment3');
		}
		tag = new Segment3(this, this._nextTagId++, start, length);
		if (!this.body) {
			this.body = tag;
		}

	} else {
		tag = new Tag(this, this._nextTagId++, ebmlID, start, length);
	}
...
Document.prototype.write = function(stream, options, callback) {
	...
	this._computePositions(function(error) {
		if (error) {
...
Document.prototype.getEBML = function() {
	return this.getFirstChildByName(schema.byName.EBML);
};

Document.prototype.getFirstSegment = function() {
	this.body = this.getFirstChildByName(schema.byName.Segment);

	return this.body;
};

Document.prototype.listSegments = function() {
