...
function addAttribute(proto, ebmlName) {
		...
		set: function(value) {
			...
			if (schemaInfo.type2 === "ebmlID") {
				...
				try {
					...
					console.log("Type of attribute=", schemaInfo);
	...
	proto["set" + ebmlName] = function(value) {
		this[name] = value
...
function addChild(proto, ebmlName) {
	...
	Object.defineProperty(proto, name + "s", {
		iterable: true,
		get: function() {
...
function oneChild(proto, ebmlName, autoCreate) {
	var ebmlID = schema.byName[ebmlName];
	if (!ebmlID) {
		throw new Error("Invalid ebmlName '" + ebmlName + "'");
	}
	...
	Object.defineProperty(proto, name, {
		...
		get: function() {
			...
			// console.log("Get '" + ebmlName + "' => " + child + " " + autoCreate);

			if (child || autoCreate !== true) {
				return child;
			}
