	...
	source.getStream({
		...
		stream.on('readable', function() {
			...
			self._write(buffer, null, function() {
				if (self._stop) {
					enabled = false;
					callback(self._invalidFormat, self.document);
					return;
				}
...
Decoder.prototype.readContent = function() {
	...
	if (leftBytes < tag.dataSize) {
		if (debugTag.enabled) {
			debugTag('got: ' + leftBytes);
			debugTag('need: ' + (tag.dataSize - leftBytes));
			debugTag('waiting for more data');
		}
...
Decoder.prototype.endContent = function(tagObj) {
	...
	while (stack.length) {
		...
		//console.log(this._fileOffset + "/" + topElement.end + " #" + topElement.tagId);
	...
	if (debugTag.enabled) {
		debugTag('read data: ' + (tagObj.data && tagObj.data.toString('hex')));
	}
