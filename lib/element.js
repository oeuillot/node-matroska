...
function Element(doc, tagId, ebmlID, start, length) {
  ...
  if (!schemaInfo) {
    console.error("Invalid schemaID ", ebmlID, "tagId=", tagId, "doc=", doc);
    throw new Error("Invalid schemaId " + ebmlID);
...
Element.prototype.getValue = function() {
  ...
  case 'u':
    var u = this.getUInt();
    if (((u === 0) || (u === 1)) && this.schemaInfo &&
        this.schemaInfo.range === "0-1") {
      u = (u > 0);
    }
    return u;
  case 'b':
...
Element.prototype.print = function(level) {
  ...
  if (this.masterType) {
    ...
    if (this.dataSize || this.data) {
      ...
      if (this.data) {
        s += "=" +
            this.data.slice(0, Math.min(32, this.data.length)).toString('hex');
...
Element.prototype._getSize = function() {
  if (!this._modified && this.start !== undefined) {
    assert(typeof (this.end) === "number", "End of #" + this.tagId +
        " is not a number");
    assert(typeof (this.start) === "number", "Start of #" + this.tagId +
        " is not a number");
  ...
  if (!this.masterType) {
    ...
    assert(typeof (dataLength) === "number", "Data size of #" + this.tagId +
        " is not a number");

    var sz2 = tools.sizeHInt(this.ebmlID) + tools.sizeVInt(dataLength) +
        dataLength;
  ...
  if (this.children) {
    this.children.forEach(function(child) {
      ...
      assert(typeof (s) === "number", "Size of #" + child.tagId +
          " is not a number");
      totalSize += s;
    });
  }
...
Element.prototype._write = function(output, source, callback) {
  ...
  if (!this.masterType) {
    ...
    if (!this.data && this._dataSource) {
      source.writeTagDataSource(output, this.dataSize, this._dataSource,
          callback);
      return;
    }
...
Element.prototype.getTagByPosition = function(position, contentOffset) {
  ...
  if (contentOffset) {
    ...
    if (!this.masterType) {
      position += tools
          .sizeVInt((this.data) ? this.data.length : this.dataSize);
...
Element.prototype.appendChild = function(child, noUpdate) {
  if (!this.masterType) {
    console.error(this);
    throw new Error("Element " + this._name + "/" + this.ebmlID + "/" +
        this.type + " is not a master type");
  }
...
Element.prototype.moveChildBefore = function(child, beforeChild) {
  ...
  if (!beforeChild) {
    ...
    if (bidx < 0) {
      throw new Error("Can not find the before child '" + beforeChild +
          "' parent=" + this);
    }
  }
