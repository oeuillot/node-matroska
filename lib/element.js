...
Element.prototype.appendChild = function(child, noUpdate) {
	if (!this.masterType) {
		throw new Error("Element " + this.name + " is not a master type");
	}
...
Element.prototype.addCRC = function() {
	if (!this.masterType) {
		throw new Error("Can not add CRC to element '" + this.name + "'");
	}
	var crc = this.getDirectChildByName(schema.byName.CRC_32);
	if (crc) {
		return crc;
	}

	crc = this.ownerDocument.createElement(schema.byName.CRC_32);
	crc.setCRCValue(0);
	this.insertBefore(crc, this.firstChild);
